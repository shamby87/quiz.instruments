...
public class Storage {
    ...
    void onAppCreate() throws IOException {
    ...
    void onAppClose() throws Exception {
    ...
    private void initLevels() {
        try {
            ...
            Reader reader = new InputStreamReader(context.getAssets().open(INSTRUMENTS_PATH));
    ...
    private <T> T loadObject(File file, Type... types) throws IOException {
        if (!file.exists()) {
            Type type = types.length > 1 ? new GType(types[0], Arrays.copyOfRange(types, 1, types.length)) : types[0];
            if (Types.isArray(type)) {
                if(!(type instanceof  Class)) {
                    throw new BugError("Invalid array type.");
                }
    ...
    private <T> T loadObject(Reader reader, Type... types) throws IOException {
        Type type = types.length > 1 ? new GType(types[0], Arrays.copyOfRange(types, 1, types.length)) : types[0];
    ...
    private void saveObject(Object object, File file) throws IOException {
        Writer writer = null;
        try {
            Files.mkdirs(file);
